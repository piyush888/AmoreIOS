...
//  Amore
//
//  Created by Kshitiz Sharma on 11/15/22.
//
...
class CustomSlider: ObservableObject {
    ...
    @SliderValue var lowHandleStartPercentage = 0.50

    var anyCancellableHigh: AnyCancellable?
    var anyCancellableLow: AnyCancellable?
    
    init(start: Double, end: Double, lhSP:Double, hHSP:Double) {
        valueStart = start
        valueEnd = end
        highHandleStartPercentage = hHSP
        lowHandleStartPercentage = lhSP
